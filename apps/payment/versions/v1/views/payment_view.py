...
from apps.payment.versions.v1.filters import PaymentTransactionFilter, ServiceTransactionFilter
...
class PaymentServiceView:
    ...
    class PaymentServiceViewSet(GenericViewSet):
        serializer_class = ServiceTransactionResponseSerializer
        queryset = ServiceTransactions.objects.all()
        action_serializers = {
            'service_payment_request': PaymentServiceRequestSerializer,
        }
        permission_classes = [IsAdminRole]
        filterset_class = ServiceTransactionFilter
